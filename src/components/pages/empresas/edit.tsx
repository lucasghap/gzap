...
const EditCompany: React.FC<EditCompanyProps> = ({ onClose, companySelected }) => {
  ...
  const { mutate, isLoading } = useMutation(
    async (data: FormInputs) => {
      ...
      await api.put(`/companies/${companySelected.id}`, unmaskedData);
    },
    {
      onError: (error: any) => {
        toast({
      ...
      <DialogPortal>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Informe os dados para editar a empresa.</DialogDescription>
          </DialogHeader>
          <Form {...form}>
